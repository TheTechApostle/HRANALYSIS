...
def get_demo_data():
    np.random.seed(42)
    n = 1470
    depts  = np.random.choice(["Research & Development","Sales","Human Resources"], n, p=[0.65,0.28,0.07])
...
with st.sidebar:
    ...
    with st.expander("⚙️ Threshold Settings", expanded=False):
        ...
        st.caption(f"LR ≥ {int(lr_thresh*100)}% → soft touch · XGB ≥ {int(xgb_thresh*100)}% → high-value")

    st.button("🔍 Predict Attrition Risk", use_container_width=True, type="primary")
...
3. Restart the app
    ...
    with g1:
        ...
        if xgb_prob is not None:
            fig_xgb, xgb_pct = make_gauge(xgb_prob, xgb_thresh, "XGBoost P(leave)", "#e74c3c")
    ...
    with g2:
        ...
        if lr_prob is not None:
            fig_lr, lr_pct = make_gauge(lr_prob, lr_thresh, "LR P(leave)", "#2e6da4")
            ...
            st.info("LR_model.pkl not loaded — add it to enable the soft-intervention tier.")
    ...
    radar_labels = ["Overtime","Job Sat.","Env. Sat.","Work-Life","Involvement","Stagnation","Compensation"]
    radar_vals = [
    ...
    for rec in sorted(recs, key=lambda r:{"urgent":0,"medium":1,"low":2}[r["priority"]]):
        css = {"urgent":"rec-card rec-urgent","medium":"rec-card rec-medium","low":"rec-card rec-low"}[rec["priority"]]
        badge = {"urgent":"🔴 Urgent","medium":"🟡 Medium","low":"🟢 Low"}[rec["priority"]]
...
# ══════════════════════════════════════════════
with tab2:
    st.markdown("<div class='section-title'>⚖️ Dual-Model Side-by-Side</div>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<div class='section-title'>Score vs Threshold Chart</div>", unsafe_allow_html=True)
    ...
    fig_cmp.update_layout(showlegend=False, template="plotly_white",
        yaxis=dict(range=[0,115], title="Attrition Probability (%)"),
    ...
    # Tier logic table
    st.markdown("---")
    st.markdown("<div class='section-title'>Tier Assignment Logic</div>", unsafe_allow_html=True)
    st.markdown(f"""
...
> Everyone who receives a high-value intervention also gets the soft touch — no one falls through.
    ...
    # Benchmark table
    st.markdown("---")
    st.markdown("<div class='section-title'>All-Model Benchmark Performance</div>", unsafe_allow_html=True)
    perf = pd.DataFrame({
...
with tab3:
    ...
    m2.metric("Attrition Count",f"{att}", delta=f"{att_rt}% rate", delta_color="inverse")
    ...
    with r1c1:
        da = demo_df.groupby(["Department","Attrition"]).size().reset_index(name="Count")
    ...
    with r3c1:
        oa = demo_df.groupby(["OverTime","Attrition"]).size().reset_index(name="Count")
    ...
    sm = demo_df.groupby("Department")[["JobSatisfaction","WorkLifeBalance"]].mean().round(2)
...
with tab4:
    ...
    lc_df = pd.DataFrame(list(lr_coef.items()),columns=["Feature","Coefficient"]).sort_values("Coefficient")
    lc_df["Direction"] = lc_df["Coefficient"].apply(lambda x:"↑ Risk driver" if x>0 else "↓ Retention signal")
    st.plotly_chart(px.bar(lc_df,x="Coefficient",y="Feature",orientation="h",
        ...
        color_discrete_map={"↑ Risk driver":"#e74c3c","↓ Retention signal":"#27ae60"},
